...
# filesystem, a partition table or a partition all mean the device carries
# somebody's data — a restored snapshot, a volume the customer attached, or this
# instance's second boot — and mkfs would destroy it.
find_blank_data_device() {
    local device root signature partitions
    root="$(root_device)"
    while read -r device; do
        [[ "/dev/$device" == "$root"* ]] && continue
        signature="$(lsblk --noheadings --nodeps --output FSTYPE,PTTYPE "/dev/$device" | tr -d ' ')" || continue
        [[ -z "$signature" ]] || continue
        # Everything below the device itself. A partitioned disk reports no
        # filesystem of its own, so without this a volume whose data sits in a
        # partition would look untouched.
        partitions="$(lsblk --noheadings --output NAME "/dev/$device" | tail -n +2)" || continue
