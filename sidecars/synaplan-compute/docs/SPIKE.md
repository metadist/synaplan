...
| Egress policy | yes | **PASS (fail-closed)** — every run is NetworkMode none; any non-empty allow-list is refused in A0–A2 (no proxy exists yet); the pinned-IP policy (`egress.CheckAllowList` / `egress.Proxy`) is kept under test for the future proxy | PASS (configurable) | Partial — network is a runner flag, not a per-run pinned-IP allow-list | **FAIL** — kernel can use host network | N/A once push/pull failed |
