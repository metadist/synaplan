...
A run with `workspace.kind=user` must carry the same `owner` (403 `workspace_not_owned`). One run at a time per folder: while a run holds a workspace, a second `POST /v1/runs` naming it — or a `DELETE` of the folder — is refused with 409 `workspace_busy` (the folder is mounted read-write, so two runs would race on the same files). The quota is checked twice — before the run against the uploaded bytes (409 `workspace_quota_exceeded`) and again once the container is gone, including cancelled runs. A finished run that left the folder above its quota is marked `failed` with reason `workspace_quota_exceeded`. A cancelled run stays `cancelled` but still gets the rollback: every entry it added (files and symlinks not present before the run) is removed so the folder is usable again. Files that existed before and were only modified are kept. Empty files and folders count as 4 KiB each so they cannot exhaust inodes for free.
