...
/**
 * Custom Naming Strategy für B-Prefix (BUSER, BMESSAGES, etc.)
 * 
 * Mappt Entity-Namen und Properties zu Tabellen/Spalten mit B-Prefix:
 * - User → BUSER
 * - Message → BMESSAGES
 * - id → BID
 * - email → BMAIL
...
class CustomNamingStrategy extends UnderscoreNamingStrategy
{
    ...
    {
        // Extrahiere kurzen Klassennamen (ohne Namespace)
        $shortName = substr($className, strrpos($className, '\\') + 1);
        
        // Entferne "Entity" Suffix falls vorhanden
        if (str_ends_with($shortName, 'Entity')) {
            $shortName = substr($shortName, 0, -6);
        }
        
        // Spezielle Plural-Mappings
        $pluralMap = [
            'Message' => 'MESSAGES',
            'Prompt' => 'PROMPTS',
            'Model' => 'MODELS',
        ...
        return str_replace('_', '', $tableName); // Keine Underscores: BUSER nicht B_USER
    }
