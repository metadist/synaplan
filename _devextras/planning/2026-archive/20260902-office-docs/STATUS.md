...
Phase T/A/B merged to `main` as
[#1685](https://github.com/metadist/synaplan/pull/1685) (2026-09-03). The
rows below still named the pre-merge branch; they are now recorded as
merged. Local CI mirror from that branch (2026-09-02): `make lint`,
`make -C backend phpstan`, unfiltered `make test` (4796 PHPUnit + 1524 Vitest),
`vue-tsc`, `doctrine:schema:validate`, and `tests/*.test.mjs` were green.
...
| ---- | ------ | ----- | ----- |
| T1 — Provider contract, consistent `tool_use` flags + migration, `ToolCallAccumulator`, `OpenAiToolShapes`, TestProvider | `feat/office-tools-v1` | done (#1685) | Dual gate; catalog matrix test |
| T2 — Chat Completions providers (Groq, OpenAICompatible, Mistral, xAI, TrustedTokens, HuggingFace) | `feat/office-tools-v1` | done (#1685) | Shared trait; `supportsToolCalling` = catalog flag |
| T3 — `/v1/chat/completions` tools pass-through (non-stream + stream), dual-gate 400 | `feat/office-tools-v1` | done (#1685) | Collabora client tools work from here |
| T4 — Server-side MCP + `web_search` loop on `/v1/chat/completions` | `feat/office-tools-v1` | done (#1685) | Injected on dual-gated models; client tools still relayed |
| T5 — OpenAI Responses API provider | `feat/office-tools-v1` | done (#1685) | `function_call` / `function_call_output`; stream deltas |
| T6 — Anthropic + Google (+ Ollama optional) | `feat/office-tools-v1` | done (#1685) | Ollama left optional (no catalog `tool_use` rows) |
| T7 — Wrap-up docs, admin badge, STATUS cross-record | `feat/office-tools-v1` | done (#1685) | Tools badge from `hasFeature('tool_use')` |

## Phase A — engine and UX quick wins

| Step | Branch | State | Notes |
| ---- | ------ | ----- | ----- |
| A0 — Converter client + `collabora/code` sidecar | `feat/office-tools-v1` | done (#1685) | `OfficeConverterClient`; profile `office` on dev + minimal + `deploy/compose.yaml`; empty `OFFICE_CONVERT_URL` by default. Platform wiring: `feat/office-collabora-sidecar` in `synaplan-platform` |
| A0-docs — LibreOffice/Collabora required for new office features | `feat/docs-office-libreoffice` in **`synaplan-docs`** | done (synaplan-docs, shipped with A0) | New `docs/office-documents.md` + cross-links (quickstart, FAQ, architecture, using-synaplan, desktop-tools). Ships with A0 so operators see the requirement before the features land |
| A1 — Thumbnails for office docs and PDFs | `feat/office-tools-v1` | done (#1685) | Messenger `async_index`; PDF via rasterizer without engine |
| A2 — Download as PDF | `feat/office-tools-v1` | done (#1685) | `GET /api/v1/files/{id}/export` + guest twin; menu hidden when engine off |
| A3 — Inline preview modal | `feat/office-tools-v1` | done (#1685) | `DocumentPreviewModal`; native WebView falls back to PDF download |
| A4 — officemaker delivers PDF | `feat/office-tools-v1` | done (#1685) | `GeneratedDocumentStore` + optional `BEXPORT`; classifier PDF only when engine on. `OfficePdfRoutingDecorator` rewrites sorter/planner PDF copy at runtime (catalog still says “no PDF” for OSS). General bounce rule does not loop create↔generate. Store keeps PDF export on follow-up edits when the conversation already asked for / attached a PDF. Snapshots unchanged (engine off in test) |
| A5 — DOCX default styling | `feat/office-tools-v1` | done (#1685) | `DocxStyleSheet` registered before HTML; footer page numbers |
| A6 — Analysable ingestion (legacy formats + structured text for xlsx/pptx) | `feat/office-tools-v1` | done (#1685) | A6a convert-then-Tika; A6b `StructuredTextExtractor` (A1 coords, slide notes) |
| A7 — Combine documents as one PDF | `feat/office-tools-v1` | done (#1685) | `POST /api/v1/files/combine`; PDF-only works without engine |

## Phase B — structured editing and merge

| Slice | Branch | State | Notes |
| ----- | ------ | ----- | ----- |
| B1 — xlsx end to end (model, renderer, revisions, tools, Groq, loop) | `feat/office-tools-v1` | done (#1685) | `SpreadsheetModel` + `XlsxRenderer` + xlsx tools + `ChatToolLoop`; flag default OFF |
| B2 — more providers + `tools` model feature | `feat/office-tools-v1` | done (#1685) | Shared with Phase T (`ToolCallingChatProviderInterface`) |
| B3 — docx | `feat/office-tools-v1` | done (#1685) | `WordModel` + `DocxRenderer` + docx tools |
| B4 — pptx | `feat/office-tools-v1` | done (#1685) | `DeckModel` + pptx tools |
| B5 — importers + fidelity report | `feat/office-tools-v1` | done (#1685) | IOFactory xlsx; best-effort docx/pptx; classifier guard default-off |
| B6 — version history UI + admin toggle | `feat/office-tools-v1` | done (#1685) | `GET/POST /files/{id}/revisions`; i18n; `documentToolsEnabled` |
| B7 — true merge on the model | `feat/office-tools-v1` | done (#1685) | `merge_documents` + combine `format` + Files UI |
