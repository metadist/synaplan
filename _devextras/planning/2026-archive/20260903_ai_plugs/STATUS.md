...
| 2026-09-09 | **S5 merged** as #1761 (incl. Copilot review fixes: reject empty import endpoint name, drop non-canonical `pic2pic`/`img2vid` import tags). |
| 2026-09-09 | **S6 implementation.** Manifest v2 `provides.plugs` parsed and validated in `PluginManifest` (unknown port / class-outside-namespace / bad key rejected). **Correction to the sprint premise:** plugin adapters are NOT tagged "by accident" — `_instanceof` is file-scoped and misses plugin services loaded via the fluent API, so plug-interface tagging moved to `registerForAutoconfiguration` in `Kernel::build()` (core + plugins, tagged once). `PlugDeclarationCheckPass` fails the boot for a tagged plugin adapter missing from `provides.plugs`; registries throw `DuplicatePlugKeyException` on a key clash. `PlugKeyStore` accepts plugin-declared keys (container param `plug.plugin_keys` from `Kernel`). `PLUGINS_DIR` override + absolute config imports let a test kernel boot against fixture plugins. Reference plugin `plugins/serper_search`; the contract test lives in the core suite (CI-safe path) with plugin PSR-4 registered in `tests/bootstrap.php`. C4 boundary test: no `serper` in `backend/src`. |
...
[`../20260907_ux_user_flows.md`](../20260907_ux_user_flows.md) §5.3.
