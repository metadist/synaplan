...
| 10 | [`10_m365_actions_and_destinations.md`](./10_m365_actions_and_destinations.md) | **Phase M: Outlook calendar write, M365 mail search, multi-destination documents** | **Active — steps M0–M10** |
...
- **Trigger vocabulary extended, contract untouched** (2026-09-07, roadmap §6 / §8.5): a trigger is an **Event** (something arrives) or a **Schedule** (a point in time); the Saved Task card and an assistant's trigger row use the same sentence pattern ([`../20260907_ux_user_flows/assistant-triggers.md`](../20260907_ux_user_flows/assistant-triggers.md)). Agent Builder S5 (`AB33`/`AB37`) creates ordinary Saved Task rows for an assistant's schedules and mail rules (`BTRIGGERCONFIG.agentTrigger`, `agentId`) and adds one **additive** key to the `inbound_email` trigger config — `filter { from[], contains[], match }`, evaluated by the ingress before a run is created; absent ⇒ every mail, exactly as today. Trigger columns remain authoritative (§3.3). Rows created by an assistant show its icon and "From assistant {name}" in the Saved tasks list and stay editable there.
...
**2026-08-18 (Phase M progress, same day):** M1, M0 and M3a–M3d implemented on `feat/m365-flow` (utterance characterization locks, DOCX `{{TOC}}` directive, Graph mail search + body fetch, merged IMAP/M365 `email_search`, flag seeded on). **C13 Dropbox pulled forward on product-owner request as step M10 and shipped**: full OAuth connector (BCONFIG group `DROPBOX`, offline tokens, admin setup guide), `DropboxDestinationProvider` on the F4 seam, planner channel `dropbox`, US-cloud label, four locales — details in [07 §4.7](./07_connectors.md). M365 connect flow verified live end-to-end (localhost redirect URI); Nextcloud image delivery verified live in chat.
