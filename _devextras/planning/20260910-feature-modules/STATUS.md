...
| S3 Gates & lazy registries (FM12–FM17) | backend: `intermezzo/s3-backend-gates-and-lazy-registries` (PR #1817, stacked on S2 PR #1816); frontend: separate PR | in progress | Split in two PRs along the mobile-impact boundary: FM12–FM14 are `backend-only`, FM15–FM17 `ota-candidate`. FM12: `Module\Http\ModuleGateListener` on `kernel.controller` (after the firewall → 401/403 keep their codes, anonymous probes learn nothing) replaces the controller with `404 {error: feature_not_configured, module, docs}` when `forRoute(_route)` finds an ABSENT module whose `MODULES.GATE_<ID>` is on; `Module\Gate\ModuleGateConfig` loads the group once per request (`ResetInterface`), malformed values read as off; `Seed\ModuleGateSeeder` inserts 12 × `GATE_<ID>=0` from `ModuleRegistry::ids()` (`app:seed` step `module-gates`). Live check on the dev stack: gate off → today's 503, gate on → the 404 body, `api_phone_verify_status` 200 and the Meta verify handshake 403 untouched, anonymous 401, runtime config shows `gated: true` on the next request without a cache clear. FM13: `ProviderRegistry` holds eight `#[AutowireLocator('app.ai.<cap>', indexAttribute: 'key')]`; every `app.ai.*` tag in `services.yaml` (61) carries `key: <getName()>`; by-name lookup instantiates only the requested provider, `getRegisteredProviderNames()` is free, listing surfaces unchanged (still instantiate all); `tests/Integration/AI/ProviderRegistryKeyTest` boots the kernel and asserts key === getName() for every tag; `debug:container --tag=app.ai.chat` shows the key column. `SkillCatalog`/`RunnerRegistry`/`DocumentToolRegistry` left eager (FM5 verdict: medium/low value) — candidates for a later step. FM14: `GET /api/v1/config/runtime` → additive `modules: {<id>: {configured, gated}}` (OpenAPI, schemas regenerated). |
