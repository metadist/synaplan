...
**Status:** In progress 2026-08-21 — M1, M0, M3a–M3d and M10 (Dropbox, pulled forward from C13 on product-owner request) implemented (full backend + frontend gate green). **2026-08-21 (channel sprint, product-owner request):** M4/M5/M6 shipped with a deliberate deviation from M2 — instead of scope tiers, the consent scope set was expanded wholesale to include `Calendars.ReadWrite` and `Mail.Send` (`MicrosoftOAuthConfig::SCOPES`); pre-expansion connections are detected via the granted scopes recorded on the connection (`Connection::getScopes()`) and degrade honestly (no calendar channel offered, sends fall back to system SMTP) until the user reconnects. Graph calendar dedup uses the `transactionId` create contract (409 = already delivered) instead of a separate `Calendars.Read` query — one mechanism, same S13 idempotency guarantee. `email_me` now prefers the user's own mailbox via Graph `Mail.Send` (`M365MailSender`, SMTP fallback). Also shipped outside this doc's scope: MCP write actions (`mcp_action` capability, per-server `allow_write` opt-in, Jira/Confluence presets — see plan 09 §3.2 successor work). Still open: M2 incremental-consent UX ("Upgrade access" button), M7 copy work, M8 (OpenCloud), M9 (OneDrive). **Branch:** `feat/m365-flow`, continued on `cursor/channel-management-planning-3d6f`.
