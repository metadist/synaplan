...
| 7 | **MCP data nodes + Skill Registry** — add a "pull data from MCP" building block to the DAG routing, and formalize every DAG block as a self-describing **Skill** so the planner prompt is generated (not hand-maintained) — the n8n-style "manage the points" answer. | [`08_mcp-data-nodes-and-skill-registry.md`](./08_mcp-data-nodes-and-skill-registry.md) | P1 | Decisions locked (2026-07-02); sprint plan absorbed into Feature 9 |
| 8 | **External data nodes ("fetch before you answer")** — three planner-placed DAG data sources: `url_fetch` (read a given URL), `email_search` (live IMAP search of the user's connected mailbox), `mcp_fetch` (call an external MCP server — customer-committed). Uniform reliability contract: timeout-bounded, SSRF-guarded, read-only, isolated failure. Includes the catalog-lite skill descriptors from Feature 7. | [`09_external-data-nodes.md`](./09_external-data-nodes.md) | P1 (MCP sprint: P0, customer) | Planned (2026-07-02) |
| 10 | **Landing streamline & lean composer (guest-first UX)** — kill guest padlocks in favour of small informative hint popovers; add 3 one-click example prompts on the empty landing (default when the blog feed is off); remove "Easy Mode" entirely and fold Model/Tools/Knowledge/Attach into the `+` menu, with the model shown as "Default" + a tiny caption. Frontend-only, all four locales. | [`10_landing-streamline-guest-ux.md`](./10_landing-streamline-guest-ux.md) | P1 | Planned (2026-07-05) |
| 11 | _TBD — to be added as we scope the rest of 4.0_ | — | — | Backlog |
