...
- **The commit type is not just style — it drives the release version.** `Cut Release Tag` (`.github/workflows/release-tag.yml`) defaults to `bump: auto`, which reads every commit since the last release tag: any `feat:` **or** `feat/…` (the shape a squash-merged PR title takes, e.g. `Feat/desktop client`) raises the minor version; everything else raises the patch version. A major version is never automatic — it requires explicitly selecting `bump: major`. Getting the type wrong on a feature commit silently ships it as a patch release.
