...
## 5. (Optional) Whisper.cpp binary (for on-box transcription)
```bash
...
mkdir -p /var/www/synaplan/backend/var/whisper && curl -fL -o /var/www/synaplan/backend/var/whisper/ggml-base.bin https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.bin && chmod 644 /var/www/synaplan/backend/var/whisper/*.bin
```
...
git clone <repo-url> synaplan && cd synaplan
cp backend/.env.example backend/.env
cp frontend/.env.example frontend/.env
```

## 7. Backend environment essentials
...
## 8. Backend install & database prep
```bash
cd /var/www/synaplan/backend
...
<VirtualHost *:80>
    ...
    DocumentRoot /var/www/synaplan/backend/public

    <Directory /var/www/synaplan/backend/public>
        AllowOverride All
        Require all granted
        FallbackResource /index.php
    </Directory>

    ProxyPassMatch ^/(.*\.php(/.*)?)$ unix:/run/php/php8.3-fpm.sock|fcgi://localhost/var/www/synaplan/backend/public
...
## 11. External services you already have
