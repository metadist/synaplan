...
# Test mode: turn OPcache and the PHP 8.4 tracing JIT fully off.
#
...
# 128M. Those defaults are correct for prod but have caused non-deterministic
# Doctrine schema-introspection failures inside `bin/console
# doctrine:migrations:migrate` when running short-lived CI/test PHP processes
# under PHP 8.4 (random "There is no table with name ..." errors across runs
# of the same migration). The base image's `synaplan-php-configure` shim only
# flips these off via the dev override when APP_ENV=dev, so APP_ENV=test
# inherits the full prod tuning.
#
# CI tests don't benefit from any of this caching anyway (every command is a
# fresh CLI process), so disable OPcache outright when APP_ENV=test. We write
# a 99-* override so it loads after every other ini in conf.d (including the
# 99-synaplan-env.ini the configure shim just wrote).
...
; Disables OPcache + JIT entirely so CI runs are deterministic and immune
; to PHP 8.4 tracing-JIT bugs that have caused random Doctrine schema
; introspection failures during `doctrine:migrations:migrate`.
opcache.enable              = 0
opcache.enable_cli          = 0
opcache.jit                 = disable
opcache.jit_buffer_size     = 0
EOF
    echo "🧪 APP_ENV=test → OPcache + JIT disabled (no caching in test runs)"
fi
