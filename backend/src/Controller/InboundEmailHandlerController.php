...
class InboundEmailHandlerController extends AbstractController
{
    ...
    /**
     * Get debug information for a handler (configuration details without passwords).
     */
    #[Route('/{id}/debug', name: 'debug', methods: ['GET'])]
    #[OA\Get(
        path: '/api/v1/inbound-email-handlers/{id}/debug',
        summary: 'Get debug information for email handler',
        description: 'Returns detailed configuration and status information for debugging (passwords are masked)',
        security: [['Bearer' => []]],
        tags: ['Inbound Email Handlers']
    )]
    #[OA\Parameter(
    ...
    #[OA\Response(
        ...
        content: new OA\JsonContent(
            properties: [
                new OA\Property(property: 'success', type: 'boolean', example: true),
                new OA\Property(property: 'handler', type: 'object'),
            ]
        )
    )]
    public function debug(int $id, #[CurrentUser] ?User $user): JsonResponse
    {
        if (!$user) {
            return $this->json(['error' => 'Not authenticated'], Response::HTTP_UNAUTHORIZED);
        }
        ...
        // Build connection string for debugging
        ...
        $connectionString = sprintf('{%s:%d/%s/%s}INBOX', $server, $port, $protocol, $securityFlag);
    ...
    /**
     * Manually trigger processing for a specific handler (for debugging).
     */
    #[Route('/{id}/process', name: 'process', methods: ['POST'])]
    #[OA\Post(
        path: '/api/v1/inbound-email-handlers/{id}/process',
        summary: 'Manually trigger email processing for a handler',
        description: 'Manually processes emails for a specific handler. Useful for debugging and testing.',
        security: [['Bearer' => []]],
        tags: ['Inbound Email Handlers']
    )]
    #[OA\Parameter(
    ...
    public function process(int $id, #[CurrentUser] ?User $user): JsonResponse
    {
        if (!$user) {
            return $this->json(['error' => 'Not authenticated'], Response::HTTP_UNAUTHORIZED);
        }
        ...
        try {
            $result = $this->handlerService->processHandler($handler);

            // Update handler status and lastChecked
            if ($result['success']) {
                $handler->setStatus('active');
            } else {
                $handler->setStatus('error');
            ...
            $this->logger->info('Manual handler processing triggered', [
                'handler_id' => $id,
                'user_id' => $user->getId(),
            ...
            $this->logger->error('Manual processing failed', [
                'handler_id' => $id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
