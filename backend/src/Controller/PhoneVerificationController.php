...
class PhoneVerificationController extends AbstractController
{
    ...
    private function findLastWhatsAppPhoneNumberId(User $user): ?string
    {
        ...
        $this->logger->debug('Searching for last WhatsApp message', [
            'user_id' => $user->getId(),
        ]);
        ...
        if (!$lastMessage) {
            $this->logger->warning('No WhatsApp messages found for user', [
                'user_id' => $user->getId(),
            ]);
        ...
        // Extract phone_number_id from metadata (stored by webhook)
        $phoneNumberId = $lastMessage->getMeta('to_phone_number_id');
        $displayPhone = $lastMessage->getMeta('to_display_phone');

        $this->logger->info('Found WhatsApp phone_number_id from last message', [
            'user_id' => $user->getId(),
            'message_id' => $lastMessage->getId(),
            'phone_number_id' => $phoneNumberId ?? 'NOT_IN_METADATA',
            'display_phone' => $displayPhone ?? 'NOT_IN_METADATA',
        ]);
