...
class mailHandler
{
    ...
    private static function createGoogleClientForUser(int $userId, string $redirectUri, array $scopes)
    {
        ...
        if (isset($app['provider']) && strtolower($app['provider']) === 'google' && !empty($app['google_client_id']) && !empty($app['google_client_secret'])) {
            if (!class_exists('Google_Client')) {
                throw new \Exception('Google client not installed');
            }
    ...
    public static function oauthStart(string $provider, string $redirectUri, int $userId, string $email = ''): array
    {
        try {
            ...
            if ($email !== '') {
                self::saveOAuthRow($userId, 'email', $email);
            }
            if ($provider === 'google') {
                if (!class_exists('Google_Client')) {
                    return ['success' => false, 'error' => 'Google client missing'];
                }
                ...
                if (!class_exists('Stevenmaguire\OAuth2\Client\Provider\Microsoft')) {
                    return ['success' => false, 'error' => 'Microsoft provider missing'];
                }
        ...
        try {
            ...
            if ($provider === 'google') {
                ...
                if (isset($token['error'])) {
                    return ['success' => false, 'error' => $token['error_description'] ?? 'OAuth error'];
                }
                ...
                if ($refresh !== '') {
                    self::saveOAuthRow($userId, 'refresh_token', $refresh);
                }
                $expiresAt = 0;
                if (isset($token['created']) && isset($token['expires_in'])) {
                    $expiresAt = ((int)$token['created']) + ((int)$token['expires_in']);
                }
                if ($expiresAt > 0) {
                    self::saveOAuthRow($userId, 'expires_at', (string)$expiresAt);
                }
                ...
                if ($token->getRefreshToken()) {
                    self::saveOAuthRow($userId, 'refresh_token', $token->getRefreshToken());
                }
                if ($token->getExpires()) {
                    self::saveOAuthRow($userId, 'expires_at', (string)$token->getExpires());
                }
    ...
    private static function getImapConfigForUser(int $userId): array
    {
        ...
        while ($row = db::FetchArr($res)) {
            switch ($row['BSETTING']) {
                case 'server': $cfg['server'] = $row['BVALUE'];
                    break;
                case 'port': $cfg['port'] = (int)$row['BVALUE'];
                    break;
                case 'protocol': $cfg['protocol'] = $row['BVALUE'];
                    break;
                case 'security': $cfg['security'] = $row['BVALUE'];
                    break;
                case 'username': $cfg['username'] = $row['BVALUE'];
                    break;
                case 'password': $cfg['password'] = $row['BVALUE'];
                    break;
            }
        }
        return $cfg;
    ...
    private static function refreshAccessTokenIfNeeded(int $userId, string $authMethod): array
    {
        try {
            $rows = self::getOAuthRow($userId);
            if (empty($rows)) {
                return ['success' => false, 'error' => 'No OAuth rows'];
            }
            $now = time();
            $expiresAt = isset($rows['expires_at']) ? (int)$rows['expires_at'] : 0;
            ...
            if ($authMethod === 'oauth_google') {
                ...
                if ($expiresAt > 0 && $now >= $expiresAt && $refresh !== '') {
                    ...
                    $exp = 0;
                    if (isset($new['created']) && isset($new['expires_in'])) {
                        $exp = ((int)$new['created']) + ((int)$new['expires_in']);
                    }
                    if ($access !== '') {
                        self::saveOAuthRow($userId, 'access_token', json_encode($new));
                    }
                    if ($exp > 0) {
                        self::saveOAuthRow($userId, 'expires_at', (string)$exp);
                    }
                } else {
            ...
            if ($authMethod === 'oauth_microsoft') {
                ...
                if ($expiresAt > 0 && $now >= $expiresAt && $refresh !== '') {
                    ...
                    $access = $new->getToken();
                    if ($access) {
                        self::saveOAuthRow($userId, 'access_token', $access);
                    }
                    if ($new->getExpires()) {
                        self::saveOAuthRow($userId, 'expires_at', (string)$new->getExpires());
                    }
                }
                if (!$access) {
                    return ['success' => false, 'error' => 'No access token'];
    ...
    public static function imapListSinceLast(int $userId, int $fallbackWindowSeconds = 86400): array
    {
        ...
        try {
            ...
            $lastSeenTs = 0;
            if ($stateRow && is_numeric($stateRow['BVALUE'])) {
                $lastSeenTs = (int)$stateRow['BVALUE'];
            }
            if ($lastSeenTs <= 0) {
                $lastSeenTs = time() - max(60, $fallbackWindowSeconds);
            }
    ...
    public static function imapDeleteMessage($message): bool
    {
        try {
            ...
            if ($folder && method_exists($folder, 'expunge')) {
                $folder->expunge();
            }
            return true;
        } catch (\Throwable $e) {
            if ($GLOBALS['debug'] ?? false) {
                error_log('imapDeleteMessage error: '.$e->getMessage());
            }
            return false;
        }
    }
    ...
    public static function imapForwardMessage($message, string $targetEmail, string $targetName = ''): bool
    {
        try {
            ...
            if ($GLOBALS['debug'] ?? false) {
                error_log('imapForwardMessage error: '.$e->getMessage());
            }
            return false;
        }
    }
    ...
    public static function processNewEmailsForUser(int $userId, int $maxOnFirstRun = 25): array
    {
        ...
        try {
            ...
            try {
                ...
                try {
                    $cnt = method_exists($messages, 'count') ? $messages->count() : (is_array($messages) ? count($messages) : 0);
            ...
            foreach ($collected as $msg) {
                try {
                    ...
                    if ($attachments && $attachments->count() > 0) {
                        foreach ($attachments as $att) {
                            $attachNames[] = $att->getName() ?: 'attachment';
                        }
                    }
                    $attachmentSection = '';
                    ...
                    if ($chosen === '' || !in_array($chosen, $allowedEmails, true)) {
                        $chosen = strtolower($defaultEmail);
                    }
