...
**Baseline.** The first successful listing of a provider (including a key added months later) records every current id in `BCONFIG` (`MODEL_DISCOVERY`) and reports none of them as pending — so enable it only after every model you already mean to add is in `BMODELS`, or that model is baselined and never reported. New ids after that baseline are **pending** until this install has a matching `BMODELS` row (including inactive/retired), an exact `ModelDiscoveryIgnoreList` entry, or a matching class rule. If the provider stops listing a pending id, it is dropped silently (and cleared from the announced map). Matching lowercases and strips Google's `models/` prefix; date snapshots are asymmetric — an undated catalog id covers later dated listings (e.g. `gpt-4o` covers `gpt-4o-2024-11-20`), and an undated upstream id covers a date-pinned catalog row, but a new dated snapshot of a date-pinned catalog row stays pending.

**Cadence.** New pending ids are Discord-posted **once** on the day they appear (`announced` map in `BCONFIG`). Open ids stay in the console every run and appear on Discord as one "Still open" summary line; every **Monday** the digest lists them in full (grouped variants) as a weekly reminder. Provider listing failures (`failingSince`) are posted once, then only on Monday until the next OK listing clears them. At most one post per calendar day across all nodes (atomic `BCONFIG` claim). A failed post releases the claim and marks nothing; `--notify` with Discord disabled still marks announced so console-only installs do not repeat forever.

**Family labels.** `ModelFamilyClassifier` tags each pending id (`New family` / `New generation` / `New line` / `New version`) from tokenised comparison against known BMODELS ids — **label only**, never a filter. Discord groups variants under a shorter root id (`gpt-6-sol` + dated/mini snapshots → one line).

**Class rules.** `ModelDiscoveryIgnoreList::CLASS_RULES` silence classes we never sell (e.g. OpenAI `ft:` fine-tunes, moderation, realtime). They are not pending and never announced; the console shows the per-provider count. A guard test asserts no `ModelCatalog` row matches a class rule — remove the rule before we start selling that class.

**Resolve a pending id:** add it to `ModelCatalog` via the normal playbook above (then seed), **or** add an exact `provider:id` entry to `ModelDiscoveryIgnoreList` with `reason` + `decidedOn`. Obsolete exact ignore entries (gone upstream, or now in `BMODELS`) are printed to the console only. Class rules are not obsolete-scanned.

**Discord** (`--notify` + `DISCORD_WEBHOOK_URL`): posts on new pending, new failures, unannounced baselines, or the Monday reminder. A "could not check" line means the key is present but the listing request failed — not the same as no key / no listing endpoint (those stay silent).
...
An entry pins the **LiteLLM value** we disagree with (both `litellm_in` and `litellm_out` for an in/out mismatch, and/or optional per-dimension keys such as `litellm_cache_read` / `litellm_cache_write` / `litellm_in_above`, in the unit the sync compares in: per 1M tokens for per_token rows, per billable unit for media rows), plus `source`, `verifiedOn` and `reason`. Because the value is pinned rather than the row, the entry silences exactly the figure a human verified and nothing else:
...
The daily drift check (`app:sync-model-prices --dry-run --fail-on-drift`) compares the **effective** cache-read and cache-write rates billing would charge (authored `cache_read_price_per_1M` / `cache_write_multiplier`, or the provider fallback from `CachePriceResolver`) and the long-context tier in `CONTEXT_PRICING` against the matching LiteLLM fields. A missing override that would silently fall back to the wrong rate is drift. A cache or tier difference alone is only reported, never written.
