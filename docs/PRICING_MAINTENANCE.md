...
**Discord** (`--notify` + `DISCORD_WEBHOOK_URL`): posts when there is at least one pending id, a provider that has a key but whose listing failed ("could not check …"), or a baseline that has not yet been announced. At most one post per calendar day across all nodes (atomic `BCONFIG` claim). A failed post releases the claim and is retried on the next run; the baseline notice is repeated until one post succeeds (or `--notify` runs with Discord disabled). A "could not check" line means the key is present but the listing request failed — not the same as no key / no listing endpoint (those stay silent).
