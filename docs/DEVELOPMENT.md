...
6. **Saved Task schedules** (`app:saved-tasks:tick`) claim due tasks with a compare-and-set and a cross-node Redis lock (`saved-tasks-tick`). The Docker scheduler role runs it every cycle after the media reaper. It is a no-op while `SAVEDTASKS / ENABLED` is globally off. Production host-cron (`cron-saved-tasks.sh` in `synaplan-platform`) is a separate PR.
