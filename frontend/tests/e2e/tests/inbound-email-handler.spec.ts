/**
 * Smoke: Inbound Email Handler CRUD API.
 * Request-only (no browser). Tests the full lifecycle of mail handler management.
 */
...
test.describe('@ci @smoke Inbound-Email-Handler API', () => {
  ...
  test('full CRUD lifecycle: create → get → list → update → delete', async ({ request }) => {
    ...
    await test.step('List handlers includes created one', async () => {
      ...
      const json = await res.json()
      expect(json.success).toBe(true)
      const found = json.handlers.find(
        (h: { id: number }) => h.id === handlerId
      )
