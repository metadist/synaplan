...
const isAutoGeneratedTitle = (title: string, widgetName: string | null, sessionId: string): boolean => {
  const shortId = sessionId.slice(-6)
...
const allChats = computed((): ChatItem[] => {
  return chatsStore.chats.map((c) => {
    if (c.widgetSession) {
      ...
      const title = c.title && !isAutoGeneratedTitle(c.title, session.widgetName, session.sessionId)
        ? c.title
        : defaultTitle
      const lastTimestamp =
