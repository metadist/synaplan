...
onMounted(() => {
  ...
  // If this message contains memory references but the store is still empty, try a best-effort fetch.
  // This prevents badges from staying in "loading..." state until a hard refresh.
  if (
    props.content.includes('[Memory:') &&
    isMemoryServiceAvailable.value &&
    memoriesStore.memories.length === 0 &&
    !memoriesStore.loading
  ) {
    memoriesStore.fetchMemories().catch(() => {
      // Silent: the UI should degrade gracefully if the service is down/unreachable
    })
  }
})
  ...
  if (html.includes('[Memory')) {
    ...
    html = html.replace(/\[Memory\s*:\s*(\d+)\]/gi, (match, memoryId) => {
      ...
      if (!isMemoryServiceAvailable.value) {
        ...
        // Generate disabled/unavailable badge (red with warning icon)
      ...
      if (memory) {
        ...
        // Escape HTML in memory data for tooltip
        const escapedKey = memory.key
          .replace(/"/g, '&quot;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;')
        const escapedValue = memory.value
          .replace(/"/g, '&quot;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;')
        const escapedCategory = memory.category.replace(/"/g, '&quot;')
