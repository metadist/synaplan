      ...
      >
        <div class="flex items-start gap-6">
          ...
          <div class="flex items-center gap-4 min-w-[300px] flex-shrink-0">
            ...
            >
              <ChatBubbleLeftRightIcon class="w-8 h-8" :style="{ color: widget.iconColor }" />
            </div>
            <div class="flex-1 min-w-0">
              <h3 class="text-lg font-semibold txt-primary truncate">Widget #{{ widget.id }}</h3>
            ...
            <div class="grid grid-cols-2 gap-x-8 gap-y-2.5">
              ...
              <div class="flex items-center gap-2.5 min-w-0">
                <div
                  ...
                  :class="widget.autoOpen ? 'bg-green-500' : 'bg-gray-400'"
                ></div>
                <span class="txt-secondary truncate">{{
                  widget.autoOpen ? 'Auto-open' : 'Manual'
                }}</span>
              </div>
              <div class="flex items-center gap-2.5 min-w-0">
              ...
              <div class="flex items-center gap-2.5 min-w-0">
                <div
                  class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                  :class="widget.defaultTheme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'"
                ></div>
                <span class="txt-secondary capitalize truncate">{{
                  widget.defaultTheme || 'light'
                }}</span>
              </div>
            </div>
