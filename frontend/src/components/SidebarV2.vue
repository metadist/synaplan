<template>
  ...
  -->
  <aside
    class="v2-sidebar-rail v2-desktop-chrome flex flex-col h-full"
    :class="expanded && 'v2-sidebar-rail--expanded'"
    data-testid="comp-sidebar-v2"
  >
    <!-- Brand logo + width toggle (icon only, like GPT) -->
    <div
      class="flex items-center flex-shrink-0 border-b border-white/[0.04]"
      :class="expanded ? 'justify-between px-3 h-[76px]' : 'flex-col justify-center gap-1 py-3'"
    >
      <img
        :src="iconSrc"
      ...
      />
      <button
        class="w-6 h-6 flex items-center justify-center rounded-md txt-secondary opacity-45 hover:opacity-100 hover:bg-black/[0.05] dark:hover:bg-white/[0.06] transition-opacity"
        :title="railToggleLabel"
        :aria-label="railToggleLabel"
        :aria-expanded="expanded"
        data-testid="btn-sidebar-v2-rail-toggle"
    ...
    <!--
      GPT-style: the chat list is the main body of the rail, directly under
      New Chat. One scroll, so every conversation is reachable without a
      second panel or a "show more" step. Other destinations sit below it.
    -->
    <SidebarChatHistory
      v-if="expanded && !isGuestMode"
      class="flex-1 min-h-[160px] border-t border-white/[0.04] mt-1"
    />

    <nav
      :class="
        expanded
          ? 'flex flex-col gap-0.5 px-2 py-1 flex-shrink-0 border-t border-white/[0.04]'
    ...
    <div
      ...
      "
      class="flex-shrink-0"
      :class="expanded ? 'px-2 py-2' : 'flex items-center justify-center py-2'"
    >
      <button
        class="v2-upgrade-btn rounded-xl"
        :class="
          expanded
            ? 'w-full min-h-[38px] flex items-center gap-2.5 px-3'
            : 'w-[72px] min-h-[48px] flex flex-col items-center justify-center gap-0.5 py-1.5'
        "
        :title="$t('nav.upgrade')"
      ...
      >
        <RocketLaunchIcon
          :class="expanded ? 'w-[18px] h-[18px] flex-shrink-0' : 'w-6 h-6'"
          aria-hidden="true"
        />
        <span
          v-if="expanded"
          class="flex-1 text-left text-[13px] font-semibold truncate"
          data-testid="text-sidebar-v2-upgrade"
        >
          {{ $t('nav.upgrade') }}
        </span>
        <span v-else class="v2-rail-label text-[10px] font-medium leading-tight">
    ...
    <div
      class="flex-shrink-0"
      :class="expanded ? 'px-2 py-2' : 'flex items-center justify-center py-4'"
    >
      <button
        ref="userBtnRef"
        class="v2-rail-icon rounded-lg"
        :class="
          expanded
            ? 'w-full min-h-[44px] flex items-center gap-2.5 px-2'
            : 'w-[72px] min-h-[48px] flex flex-col items-center justify-center gap-0.5 py-1.5'
        "
      ...
      >
        <div
          class="w-8 h-8 rounded-full surface-chip flex items-center justify-center text-xs font-semibold flex-shrink-0"
        >
          {{ initials }}
        </div>
        <span
          v-if="expanded"
          class="flex-1 min-w-0 text-left text-[12px] font-medium truncate"
          data-testid="text-sidebar-v2-account"
        >
...
/**
 * The rail is open whenever the user left it open. Desktop chrome is already
 * hidden below 768×520 (phone layout / drawer), so a second width lock would
 * only hide the chat list on ordinary laptop windows.
 */
const expanded = computed(() => sidebarStore.railExpanded)

const railToggleLabel = computed(() =>
  expanded.value ? t('nav.collapseSidebar') : t('nav.expandSidebar')
)

/**
 * The chat list itself is the history, so a "History" entry that opens a modal
 * of the same chats is pure noise. Dropped from the rail entirely; phone chrome
 * still reaches the sheet through its own drawer button.
 */
const visibleNavItems = computed(() => navItems.value.filter((item) => item.path !== '/'))
...
const handleNavClick = (item: NavItem) => {
  ...
  if (item.path === '/') {
    // Only reachable from phone chrome now (the rail hides this entry), where
    // the modal sheet remains the history surface.
    closeFlyout()
