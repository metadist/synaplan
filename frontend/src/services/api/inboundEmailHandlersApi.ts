...
export const inboundEmailHandlersApi = {
  ...
  async debug(id: string): Promise<{
    ...
    }>(`/api/v1/inbound-email-handlers/${id}/debug`, { method: 'GET' })
  },

  /**
   * Manually trigger email processing for a handler (for debugging)
   */
  async process(id: string): Promise<{
    success: boolean
    processed: number
    errors: string[]
  }> {
    return httpClient<{ success: boolean; processed: number; errors: string[] }>(
      `/api/v1/inbound-email-handlers/${id}/process`,
      { method: 'POST' }
    )
  },
}
