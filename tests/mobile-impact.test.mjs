...
test('local development, deployment, and test tooling does not trigger an app release', () => {
  const toolingPaths = [
    ...
    'deploy/host/caddy/Caddyfile.domain',
    'deploy/aws/packer/synaplan.pkr.hcl',
    'deploy/aws/cloudformation/synaplan-new-vpc.yaml',
    'deploy/azure/packer/synaplan.pkr.hcl',
    'deploy/azure/scripts/firstboot.sh',
    'deploy/azure/systemd/synaplan.service',
    'deploy/azure/backup/VMSnapshotScriptPluginConfig.json',
    'deploy/azure/arm/mainTemplate.json',
    'deploy/azure/arm/createUiDefinition.json',
